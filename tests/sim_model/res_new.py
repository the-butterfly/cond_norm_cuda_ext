...
class CN_thres(nn.Module):
    def __init__(self, planes=64, std=1.0, minl=1.0, eps=1e-5, thres=0):
        super().__init__()
        self.planes = planes
        self.std = std
        self.minl = minl
    ...
    def forward(self, x):
        ...
        norm = torch.sqrt(torch.mean(x**2, dim=[2,3], keepdim=True) + self.eps)
        x = self.std * x / torch.max(norm, self.minl*torch.ones_like(norm))
        x = x + (x_mean < self.thres).float()
        return x


class CN_linear(nn.Module):
    def __init__(self, planes=64, std=1.0, minl=1.0, eps=1e-5):
        super().__init__()
        self.planes = planes
        self.std = std
        self.minl = minl
        self.eps = eps
        self.weight = Parameter(torch.Tensor(1,planes,1,1))
        self.bias = Parameter(torch.Tensor(1,planes,1,1))
        init.normal_(self.weight, 0, 1e-3)
        init.normal_(self.bias, 0, 1e-3)
    ...
    def forward(self, x):
        ...
        norm = torch.sqrt(torch.mean(x**2, dim=[2,3], keepdim=True) + self.eps)
        x = self.std * x / torch.max(norm, self.minl*torch.ones_like(norm))
        x = x + self.weight * x_mean + self.bias
        return x
...
def ResNet34(nclass=10, nc=1):
    return ResNet(BasicBlock, [3,4,6,3], num_classes=nclass, nc=nc)

def ResNet50(nclass=10, normlayer=nn.BatchNorm2d):
